...
public class UserController {
    ...
    public ResponseEntity<ResponseUtilisateur> inscription(@RequestPart(value = "image", required = false) MultipartFile image, @Valid @RequestPart("user") SaveUtilisateurDTO saveUtilisateurDTO) {
        return ResponseEntity.status(200).body(this.utilisateurService.save(saveUtilisateurDTO, image));
    }
    ...
    public ResponseEntity<ResponseUtilisateur> updateUser(@PathVariable("id")UUID id, @Valid @RequestPart("user") UpdateUtilisateurDTO updateUtilisateurDTO, @RequestPart(value = "image", required = false) MultipartFile image) {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(this.utilisateurService.update(id, updateUtilisateurDTO, image));
    }
